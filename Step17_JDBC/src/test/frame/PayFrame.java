...
public class PayFrame extends JFrame implements ActionListener, PropertyChangeListener {
	//필드
	   ...
	   public PayFrame() {
	      ...
	      String[] colNames= {"지출 순서", "지출 항목", "지출 내용", "가격", "합계"};
	   ...
	   public void actionPerformed(ActionEvent e) {
		...
		public void propertyChange(PropertyChangeEvent evt) {
			...
			if(evt.getPropertyName().equals("tableCellEditosr") && !table.isEditing()) {
				//현재 선택된 row의 정보를 읽어와서 수정 반영 한다.          true&false
				int selectedRow=table.getSelectedRow();
				int num=(int)model.getValueAt(selectedRow, 0);
				String name=(String)model.getValueAt(selectedRow, 1);
				String contents=(String)model.getValueAt(selectedRow, 2);
				int price=(int)model.getValueAt(selectedRow, 3);
				int total=(int)model.getValueAt(selectedRow, 4);
				//MemberDto 객체에 담고
				PayDto dto=new MemberDto(num, name, contents, price, total);
