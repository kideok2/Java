...
import test.dto.PayDto;
import test.util.DBConnect;

public class PayDao {

	// 1. 전체 회원의 목록을 리턴하는 메소드
	// MainClass01을참고하여 메소드를 완성하세요
	// Step12_UtilClass 에 MainClass08참고
   public List<PayDto> getList(){
      ...
      try {
         ...
         //실행할 sql문 구성하기 
         String sql="SELECT num, name, contents, price, total"
               + " FROM member"
               + " ORDER BY num ASC";
	...
	// 2. 특정 회원 한 명의 정보를 리턴하는 메소드
	public PayDto getData(int num) {
		//MainClass11 을 참고해서 메소드를 완성해 보세요.
		Connection conn=null;
		PreparedStatement pstmt=null;
		ResultSet rs=null;
		PayDto dto=null;
		try {
			conn=new DBConnect().getConn();
			String sql="SELECT name, contents, price"
					+ " FROM pay"
					+ " WHERE num=?";
			pstmt=conn.prepareStatement(sql);
			// ? 에 바인딩 할 내용이 있어야 한다.
			...
			if(rs.next()) {				
				...
				dto.setName(name);
				//ResultSet 으로 부터 내용을 얻어내서 담는다.
				dto.setContents(contents);
				//ResultSet 으로 가격을 얻어내서 담는다
				dto.setPrice(price);
				//리턴?
				return dto;
					}
		...
		//회원 한 명의 정보를 추가하고 성공여부를 리턴하는 메소드
		public boolean insert(PayDto dto) {
		Connection conn=null;
		PreparedStatement pstmt=null;
		...
		//회원 한 명의 정보를 수정하고 성공여부를 리턴하는 메소드
		public boolean update(PayDto dto) {
		Connection conn=null;
		PreparedStatement pstmt=null;
		int updatedRowCount=0;
		try {
			...
			//실행 후 메소드가 리턴해주는 변화된 행의 갯수를 지역변수에 담는다. -②2
		    updatedRowCount=pstmt.executeUpdate();
		} catch(Exception e) {
			e.printStackTrace();
		}finally {
		...
		//회원 한 명의 정보를 삭제하고 성공여부를 리턴하는 메소드
		public boolean delete(int num) { //삭제할 번호만 넘겨주면 된다.
		Connection conn=null;
		PreparedStatement pstmt=null;
		//변화된(추가, 수정, 삭제) 행의 갯수를 담을 지역변수를 미리 만들어 둔다. -①1
		int updatedRowCount=0;
		try {
			//DBConnect 객체를 이용해서 Connection 객체의 참조값을 얻어온다.
			conn=new DBConnect().getConn();
